...
export default function Sales() {
            ...
            setSales([
                {
                    ...
                    items: [
                        { product: 'p1', productName: 'Pastel de Bodas', quantity: 1, unitPrice: 400, subtotal: 400 }
                    ],
                    subtotal: 400,
                    tax: 50,
                    discount: 0,
                    discountType: 'none',
                    paymentMethod: 'cash',
                    seller: { _id: 'u1', name: 'Kat', email: 'kat@test.com' },
                    customer: { name: 'Juan Pérez' }
                },
                {
                    _id: 's2',
                    invoiceNumber: 'FACT-2026-000002',
                    total: 120,
                    status: 'paid',
                    createdAt: new Date().toISOString(),
                    items: [
                        { product: 'p2', productName: 'Cupcakes Mix', quantity: 4, unitPrice: 25, subtotal: 100 }
                    ],
                    subtotal: 100,
                    tax: 20,
                    discount: 0,
                    ...
                    paymentMethod: 'card',
                    seller: { _id: 'u1', name: 'Kat', email: 'kat@test.com' },
                    customer: { name: 'Venta Mostrador' }
                }
            ]);
            setStats({
                today: { total: 570, count: 2 },
                month: { total: 12500, count: 84 }
            });
            setTotalPages(1);
            setLoading(false);
        } else {
    ...
    }, [page, startDate, endDate, search, isDemo]);
    ...
    const fetchSales = async () => {
        try {
            ...
            if (!isDemo) toast.error('Error al cargar historial de ventas');
        } finally {
            setLoading(false);
        }
    };
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="card-premium p-6 flex items-center gap-6 border-l-4 border-green-500">
                    ...
                    <div>
                        ...
                        <h3 className="text-3xl font-black text-slate-900 mt-1 italic tracking-tighter">
                            ${stats?.today.total.toLocaleString('es-MX', { minimumFractionDigits: 2 })}
                        </h3>
                        <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-widest">{stats?.today.count} transacciones</p>
                    </div>
                </div>
                <div className="card-premium p-6 flex items-center gap-6 border-l-4 border-primary-500">
                    ...
                    <div>
                        ...
                        <h3 className="text-3xl font-black text-slate-900 mt-1 italic tracking-tighter">
                            ${stats?.month.total.toLocaleString('es-MX', { minimumFractionDigits: 2 })}
                        </h3>
                        <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-widest">{stats?.month.count} transacciones</p>
                    </div>
                </div>
            </div>
