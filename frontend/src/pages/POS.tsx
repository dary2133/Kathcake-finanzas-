...
import type { Product, CartItem, Setting, Category, Customer, Sale } from '../types';
...
export default function POS() {
    ...
    const addToCart = (product: Product) => {
        ...
        const existing = cart.find((item: CartItem) => item._id === product._id);
        if (existing) {
            setCart(cart.map((item: CartItem) => item._id === product._id ? { ...item, quantity: item.quantity + 1 } : item));
        } else {
    ...
    const fetchRegisterStats = async () => {
        try {
            ...
            if (res.data.success) {
                setRegisterStats({
                    ...res.data.data,
                    total: res.data.data.totalSales // Mapping backend totalSales to local total
                });
            }
        } catch (err) {
    ...
    return (
        <div className="flex flex-col xl:flex-row gap-6 h-[calc(100vh-100px)] animate-fade-in text-slate-900 overflow-hidden">
            <div className="flex-1 flex flex-col gap-4 min-w-0 h-full">
                    ...
                    <div className="flex items-center gap-6 w-full sm:w-auto">
                        <div className="relative flex-1 sm:w-64">
                            ...
                            <input
                                ...
                                className="input-field pl-12 h-12 text-sm font-bold bg-slate-50 border-none shadow-inner"
                                value={search}
                    ...
                    <div className="flex items-center gap-4 w-full sm:w-auto justify-end">
                        <div className="relative register-dropdown-container">
                            <button
                                ...
                                className={`h-12 px-5 rounded-xl flex items-center gap-3 font-black uppercase tracking-widest text-[9px] transition-all border-2 ${isCashOpen ? 'bg-slate-900 border-slate-900 text-white shadow-lg shadow-slate-900/20' : 'bg-white border-red-100 text-red-500 hover:bg-red-50'}`}
                            >
                        ...
                        <button
                            ...
                            className="h-12 px-5 rounded-xl flex items-center gap-3 font-black uppercase tracking-widest text-[9px] bg-emerald-500 text-white shadow-lg shadow-emerald-500/20 hover:scale-105 active:scale-95 transition-all"
                        >
                        ...
                        <button
                            ...
                            className="h-12 px-5 rounded-xl flex items-center gap-3 font-black uppercase tracking-widest text-[9px] bg-red-500 text-white shadow-lg shadow-red-500/20 hover:scale-105 active:scale-95 transition-all"
                        >
                ...
                <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide px-2">
                    <button
                        onClick={() => setCategory('')}
                        className={`px-6 py-3 rounded-xl text-[9px] font-black uppercase tracking-[0.2em] transition-all border-2 whitespace-nowrap ${category === ''
                    ...
                    {categories.map((cat: Category) => (
                        <button
                            ...
                            className={`px-6 py-3 rounded-xl text-[9px] font-black uppercase tracking-[0.2em] transition-all border-2 whitespace-nowrap ${category === cat.name
                                ? 'bg-primary-600 border-primary-600 text-white shadow-2xl shadow-primary-500/30'
                                : 'bg-white border-transparent text-slate-400 hover:bg-slate-50 hover:text-slate-900'
                                }`}
                        >
                ...
                <div className="flex-1 overflow-y-auto grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6 pr-4 pb-20 custom-scrollbar">
                    {products.map((product: Product) => {
                        const inCart = cart.find((item: CartItem) => item._id === product._id)?.quantity || 0;
                        ...
                        return (
                            <div key={product._id} className="group relative">
                                <button
                                    ...
                                    className={`w-full bg-white rounded-2xl p-2.5 flex flex-col gap-2 shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)] hover:shadow-[0_12px_30px_-10px_rgba(0,0,0,0.1)] transition-all duration-300 active:scale-95 text-center overflow-hidden border border-slate-100 hover:border-slate-200`}
                                >
                                    <div className="aspect-square w-full bg-slate-50/50 rounded-xl overflow-hidden border border-slate-100/50 flex items-center justify-center relative group-hover:bg-slate-50 transition-colors duration-500">
                                        <ChefHat className="w-8 h-8 text-slate-100 group-hover:text-slate-200 transition-colors absolute" />
                                        {product.images?.[0]?.url && (
                                            <img
                                                src={product.images[0].url}
                                                className="w-full h-full object-contain p-1.5 relative z-[1] group-hover:scale-105 transition-transform duration-500"
                                                alt={product.name}
                                            />
                                        )}
                                        {inCart > 0 && (
                                            <div className="absolute top-1.5 left-1.5 bg-slate-900 text-white text-[9px] font-black w-5 h-5 rounded-full flex items-center justify-center shadow-lg z-[2]">
                                                {inCart}
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    <div className="flex flex-col gap-0.5 items-center pb-1">
                                        <span className="text-sm font-black text-slate-900 tracking-tight leading-none uppercase italic">
                                            {currency}{product.price.toLocaleString()}
                                        </span>

                                        <h3 className="font-medium text-slate-500 text-[10px] tracking-tight line-clamp-1 px-1">
                                            {product.name}
                                        </h3>

                                        <div className={`mt-1 px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-widest ${availableStock > 0
            ...
            <div className="w-full xl:w-[500px] h-full flex flex-col gap-6 bg-white rounded-[3rem] border border-slate-100 shadow-[0_40px_100px_-20px_rgba(0,0,0,0.1)] relative overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto px-10 space-y-4 custom-scrollbar">
                    {cart.length === 0 ? (
                        ...
                        cart.map((item: CartItem) => (
                            <div key={item._id} className="flex gap-3 animate-slide-up group items-center py-2 border-b border-slate-50 last:border-0">
                                <div className="w-12 h-12 bg-slate-50 rounded-xl overflow-hidden shrink-0 relative group-hover:scale-105 transition-transform duration-500">
                                    {item.images?.[0]?.url ? (
                                ...
                                <div className="flex-1 min-w-0">
                                    <p className="font-black text-slate-900 uppercase italic tracking-tighter leading-tight truncate text-[11px]">{item.name}</p>
                                    <div className="flex items-center gap-2 mt-1.5">
                                        <div className="flex items-center bg-white border border-slate-200 p-0.5 rounded-lg">
                                            <button onClick={() => {
                                                const newQty = item.quantity - 1;
                                                if (newQty === 0) setCart(cart.filter((i: CartItem) => i._id !== item._id));
                                                else setCart(cart.map((i: CartItem) => i._id === item._id ? { ...i, quantity: newQty } : i));
                                            }} className="p-0.5 hover:bg-slate-50 rounded text-slate-400 transition-all"><Minus className="w-2.5 h-2.5" /></button>
                                            <span className="w-6 text-center text-[10px] font-black text-slate-900">{item.quantity}</span>
                                            <button
                                                onClick={() => {
                                                    setCart(cart.map((i: CartItem) => i._id === item._id ? { ...i, quantity: i.quantity + 1 } : i));
                                                }}
                                                className="p-0.5 hover:bg-slate-50 rounded text-slate-400 transition-all"
                                            >
                                                <Plus className="w-2.5 h-2.5" />
                                            </button>
                                        </div>
                                        <p className="text-[9px] font-black text-primary-500 italic opacity-60">x {currency}{item.price.toFixed(0)}</p>
                                    </div>
                                </div>
                                <div className="text-right flex flex-col items-end gap-1">
                                    <button
                                        onClick={() => setCart(cart.filter((i: CartItem) => i._id !== item._id))}
                                        className="p-2 text-red-500 bg-white hover:bg-red-500 hover:text-white rounded-xl transition-all border border-red-100 hover:border-red-500 shadow-sm group/trash"
                                        title="Eliminar producto"
                                    >
                                        <Trash2 className="w-3.5 h-3.5 group-hover/trash:scale-110 transition-transform" />
                                    </button>
                                    <p className="font-black text-slate-900 text-sm tracking-tighter italic leading-none">{currency}{(item.price * item.quantity).toFixed(0)}</p>
                                </div>
                            </div>
                        ))
                    )}
                ...
                <div className="p-4 mt-auto bg-slate-900 border-t border-slate-800 relative">
                    <div className="absolute top-0 left-0 w-full h-[2px] bg-gradient-to-r from-primary-500 to-accent-500"></div>

                    <div className="flex justify-between items-center mb-4 px-1">
                        <div className="flex gap-6">
                            <div>
                                <p className="text-[7px] font-black text-slate-500 uppercase tracking-widest italic leading-none">Subtotal</p>
                                <p className="text-sm font-black text-white italic tracking-tighter mt-1">{currency}{subtotal.toLocaleString()}</p>
                            </div>
                            <div>
                                <p className="text-[7px] font-black text-slate-500 uppercase tracking-widest italic leading-none">ITBIS (18%)</p>
                                <p className="text-sm font-black text-white italic tracking-tighter mt-1">{currency}{tax.toLocaleString()}</p>
                            </div>
                        </div>
                        <div className="text-right">
                            <span className="text-[7px] font-black text-primary-400 uppercase tracking-widest italic leading-none block">Total</span>
                            <h3 className="text-3xl font-black text-white italic tracking-tight leading-none mt-1">{currency}{total.toLocaleString()}</h3>
                        </div>
                    </div>
                    ...
                    <button
                        ...
                        className="w-full h-12 bg-primary-600 hover:bg-primary-500 disabled:bg-slate-800 disabled:opacity-50 text-white rounded-xl font-black uppercase tracking-[0.2em] text-[10px] shadow-xl shadow-primary-500/10 active:scale-[0.98] transition-all flex items-center justify-center gap-2 group"
                    >
                        Continuar Venta
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="relative">
                                    ...
                                    <select
                                        ...
                                        className="w-full h-14 pl-12 pr-6 bg-slate-50 border-2 border-slate-100 rounded-2xl font-bold text-sm appearance-none cursor-pointer focus:border-primary-500 transition-all"
                                    >
                                        <option value="Cliente de Mostrador">Cliente de Mostrador</option>
                                        {customers.map((c: any) => (
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => setReceivedAmount(e.target.value)}
                            ...
                            <div className="space-y-3 pt-2 text-left">
                                ...
                                <div className="relative group">
                                    <input
                                        type="text"
                                        value={saleName}
                                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSaleName(e.target.value)}
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFreeSaleData({ ...freeSaleData, name: e.target.value })}
                                    />
                                </div>
                            </div>
                            <div className="grid grid-cols-2 gap-10">
                                <div className="space-y-3">
                                    ...
                                    <input
                                        ...
                                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFreeSaleData({ ...freeSaleData, price: e.target.value })}
                                    />
                                </div>
                                <div className="space-y-3">
                                    ...
                                    <input
                                        ...
                                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFreeSaleData({ ...freeSaleData, quantity: parseInt(e.target.value) })}
                                    />
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="relative bg-white rounded-[2.5rem] w-full max-w-2xl overflow-hidden shadow-2xl animate-scale-in flex flex-col max-h-[90vh]">
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        type="number"
                                        value={countedCash}
                                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => setCountedCash(e.target.value)}
                                    ...
                                    />
                                    {countedCash && (
                                        <div className={`absolute right-6 top-1/2 -translate-y-1/2 px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest ${Math.abs((parseFloat(countedCash || '0') - (parseFloat(initialCash || '0') + registerStats.cash))) < 1 ? 'bg-emerald-500 text-white' : 'bg-amber-500 text-white'
                                            }`}>
                        ...
                        <div className="p-8 bg-slate-50 border-t border-slate-100 flex gap-4">
                            ...
                            <button
                                ...
                                disabled={isSubmitting}
                                className="flex-[2] h-14 bg-red-500 hover:bg-red-600 disabled:opacity-50 text-white rounded-2xl font-black uppercase tracking-widest text-xs shadow-lg shadow-red-500/20 transition-all"
                            >
                                {isSubmitting ? 'Cerrando...' : 'Cerrar Caja Definitivamente'}
                            </button>
                        </div>
                    </div>
                </div>
                        ...
                        <div className="p-8 overflow-y-auto custom-scrollbar space-y-6">
                            ...
                            <div className="space-y-2">
                                ...
                                <input
                                    ...
                                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => setExpenseForm({ ...expenseForm, date: e.target.value })}
                            ...
                            <div className="space-y-2">
                                ...
                                <select
                                    ...
                                    onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setExpenseForm({ ...expenseForm, category: e.target.value })}
                            ...
                            <div className="space-y-2">
                                ...
                                <input
                                    ...
                                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => setExpenseForm({ ...expenseForm, name: e.target.value })}
                                    className="w-full h-12 px-4 border border-slate-200 rounded-xl font-bold text-sm outline-none focus:border-red-500 transition-colors"
                                />
                            </div>
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => setExpenseForm({ ...expenseForm, provider: e.target.value })}
            ...
            {/* Receipt container for printing (off-screen or managed by print media query) */}
            <div className="hidden print:block">
                {lastSale && <Invoice sale={lastSale} settings={settings} sellerName={currentUser?.name} />}
            </div>
        </div>
    );
}
