...
export default function POS() {
    ...
    const handleCheckout = async () => {
        ...
        try {
            // Filtrar items válidos (los que tienen ID real de base de datos)
            const dbItems = cart.filter(item => !item._id.toString().startsWith('free-'));

            // Si hay items "libres" (free-), por ahora no los enviamos al backend de ventas estructuradas
            // O idealmente el backend debería soportarlos.
            // Para solucionar "no guarda ventas", asegurémonos de enviar lo que el backend espera.

            if (dbItems.length === 0) {
                toast.error('La canasta solo tiene items libres no registrados. Agregue un producto del inventario.');
            ...
            if (res.data.success) {
                ...
                // Actualizar info de la caja si es necesario
                fetchRegisterStats();
            }
        } catch (error: any) {
    ...
    return (
        <div className="flex flex-col xl:flex-row gap-10 h-[calc(100vh-160px)] animate-fade-in text-slate-900 overflow-hidden">
            <div className="flex-1 flex flex-col gap-8 min-w-0 h-full">
                <div className="flex flex-col sm:flex-row items-center justify-between gap-6 bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
                    ...
                    <div className="flex items-center gap-4 w-full sm:w-auto justify-end">
                        <button
                            ...
                            className={`h-14 px-6 rounded-2xl flex items-center gap-3 font-black uppercase tracking-widest text-[9px] transition-all border-2 ${isCashOpen ? 'bg-slate-900 border-slate-900 text-white' : 'bg-white border-red-100 text-red-500 hover:bg-red-50'}`}
                        >
                ...
                <div className="flex-1 overflow-y-auto grid grid-cols-2 md:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-3 pr-4 pb-20 custom-scrollbar">
                    {products.map(product => (
                        <div key={product._id} className="group relative">
                            <button
                                onClick={() => addToCart(product)}
                                className="w-full card-premium p-1.5 flex flex-col gap-1.5 border-none shadow-sm hover:shadow-lg transition-all h-full bg-white active:scale-95 text-left overflow-hidden ring-0 hover:ring-2 hover:ring-primary-500/20"
                            >
                                <div className="absolute top-1.5 right-1.5 z-10">
                                    <div className="bg-slate-900/90 backdrop-blur-md px-1.5 py-0.5 rounded text-white font-black italic text-[9px] tracking-tighter shadow-xl">
                                        {currency}{product.price}
                                    </div>
                                </div>
                                <div className="h-14 w-full bg-slate-50 rounded-lg overflow-hidden border border-slate-100/50 flex items-center justify-center relative group-hover:bg-primary-50 transition-colors">
                                    <ChefHat className="w-6 h-6 text-slate-100 group-hover:text-primary-100 transition-colors absolute" />
                                ...
                                <div>
                                    <h3 className="font-black text-slate-900 text-[9px] uppercase italic tracking-tighter leading-tight group-hover:text-primary-600 transition-all line-clamp-2 min-h-[2.2em]">{product.name}</h3>
                                    <div className="flex items-center justify-between mt-1">
                                        <div className="min-w-0 flex-1">
                                            <p className={`text-[8px] font-black uppercase tracking-widest ${product.stock > 10 ? 'text-emerald-500' : 'text-amber-500'}`}>
                                                {product.stock} disp.
                                            </p>
                                        </div>
                                        <div className="w-5 h-5 bg-slate-50 rounded flex items-center justify-center text-slate-400 group-hover:bg-primary-500 group-hover:text-white transition-all shadow-inner group-hover:shadow-lg group-hover:shadow-primary-500/20 shrink-0">
                    ...
                    <button className="card-premium border-2 border-dashed border-slate-200 bg-slate-50 flex flex-col items-center justify-center gap-3 text-slate-400 hover:border-primary-300 hover:text-primary-500 hover:bg-white transition-all order-first cursor-pointer group p-6">
                        <div className="w-12 h-12 rounded-full border-2 border-dashed border-current flex items-center justify-center group-hover:rotate-90 transition-transform duration-500">
                            <PlusCircle className="w-6 h-6" />
                        </div>
                        <span className="font-black uppercase tracking-widest text-[8px] italic">Crear Producto</span>
                    </button>
                </div>
            </div>
                ...
                <div className="fixed inset-0 z-[90] flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-white rounded-[2rem] w-full max-w-lg overflow-hidden shadow-2xl animate-scale-in flex flex-col">
                        <div className="p-10 text-center space-y-6">
                            ...
                            <div className="space-y-2 pt-2 text-left">
                                ...
                                <input
                                    ...
                                    className="w-full h-14 px-6 bg-white border-2 border-slate-200 rounded-xl font-bold text-slate-900 focus:border-slate-900 transition-all outline-none text-sm placeholder:text-slate-300"
                                />
                            </div>
                        ...
                        <div className="p-6 border-t border-slate-100 flex flex-col gap-3 bg-white">
                            <button
                                onClick={async () => {
                                    if (saleName && lastSale?._id) {
                                        // Save name silently
                                        try { await axios.patch(`/api/sales/${lastSale._id}/details`, { customerName: saleName }); } catch (e) { }
                                    }
                                    setIsSuccessModalOpen(false);
                                    setSaleName('');
                                    setIsFreeSaleModalOpen(true);
                                }}
                                className="w-full h-14 bg-slate-900 hover:bg-slate-800 text-white rounded-xl font-black uppercase tracking-widest text-xs transition-all shadow-lg active:scale-[0.98]"
                            >
                                Nueva venta libre
                            </button>
                            <button
                                ...
                                className="w-full h-14 bg-white border-2 border-slate-200 hover:border-slate-300 text-slate-900 rounded-xl font-black uppercase tracking-widest text-xs transition-all"
                            >
                                Ir a movimientos
                            </button>
                        </div>
