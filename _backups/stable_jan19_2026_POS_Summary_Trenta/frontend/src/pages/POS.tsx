...
export default function POS() {
    ...
    return (
        <div className="flex flex-col xl:flex-row gap-10 h-[calc(100vh-160px)] animate-fade-in text-slate-900 overflow-hidden">
            ...
            {/* MODAL RESUMEN DE CAJA (ESTILO TRENTA) */}
            {isSummaryModalOpen && (
                <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
                    <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-md" onClick={() => setIsSummaryModalOpen(false)}></div>
                    <div className="relative bg-[#F8FAFC] rounded-[2rem] w-full max-w-md overflow-hidden shadow-2xl animate-scale-in flex flex-col max-h-[90vh]">
                        {/* Header */}
                        <div className="p-6 flex items-center justify-between">
                            <h3 className="text-xl font-bold text-[#1E293B]">Registros realizados</h3>
                            <button
                                onClick={() => setIsSummaryModalOpen(false)}
                                className="w-8 h-8 rounded-full bg-slate-900 flex items-center justify-center text-white hover:scale-110 transition-transform"
                            >
                                <X className="w-4 h-4" />
                            </button>
                        </div>
                        ...
                        <div className="px-6 pb-6 space-y-3 overflow-y-auto">
                            {/* Medios de Pago */}
                            {[
                                { label: 'Efectivo', amount: registerStats.cash, icon: <Banknote className="w-4 h-4" /> },
                                { label: 'Tarjeta', amount: registerStats.card, icon: <CreditCard className="w-4 h-4" /> },
                                { label: 'Transferencia bancaria', amount: registerStats.transfer, icon: <ArrowLeftRight className="w-4 h-4" /> },
                                { label: 'Otro', amount: registerStats.other, icon: <Wallet className="w-4 h-4" /> },
                            ].map((item, idx) => (
                                <div key={idx} className="bg-white border border-slate-100 rounded-xl p-4 flex items-center justify-between hover:border-slate-300 transition-colors shadow-sm">
                                    <div className="flex items-center gap-3">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <span className="font-bold text-slate-900">{currency}{item.amount.toLocaleString()}</span>
                            ...
                            <div className="bg-white border border-slate-100 rounded-xl p-4 flex items-center justify-between shadow-sm">
                                <div className="flex flex-col">
                                    <span className="font-bold text-slate-900 text-sm">Apertura</span>
                                    <span className="text-xs text-slate-400 font-medium">{registerData?.openedBy?.name || 'Usuario'}</span>
                                </div>
                                <div className="bg-blue-50 text-blue-500 px-3 py-1 rounded-lg text-[10px] font-bold">
                                    {registerData?.openedAt ? new Date(registerData.openedAt).toLocaleString('es-DO', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : '---'}
                                </div>
                            </div>
                        </div>

                        {/* Footer (Resumen del turno) */}
                        <div className="mt-auto bg-white border-t border-slate-100 p-6 shadow-[0_-10px_20px_-5px_rgba(0,0,0,0.05)]">
                            <div className="flex items-center justify-between mb-4 group cursor-pointer">
                                <span className="font-bold text-slate-900 text-sm">Resumen del turno</span>
                                <ChevronDown className="w-4 h-4 text-slate-400 group-hover:text-slate-900 transition-colors" />
                            </div>

                            <div className="space-y-2">
                                <div className="flex justify-between text-xs">
                                    <span className="text-slate-400 font-medium">Total ventas</span>
                                    <span className="text-slate-600 font-bold">{currency}{(registerStats as any).totalSales?.toLocaleString()}</span>
                                </div>
                                <div className="flex justify-between text-xs">
                                    <span className="text-slate-400 font-medium">Total abonos</span>
                                    <span className="text-slate-600 font-bold">{currency}0</span>
                                </div>
                                <div className="flex justify-between text-xs">
                                    <span className="text-slate-400 font-medium">Total gastos</span>
                                    <span className="text-slate-600 font-bold">{currency}{(registerStats as any).totalExpenses?.toLocaleString()}</span>
                                </div>
                                <hr className="border-slate-50 my-2" />
                                <div className="flex justify-between text-sm">
                                    <span className="text-slate-900 font-bold">Balance</span>
                                    <span className="text-slate-900 font-black">{currency}{(registerStats as any).balance?.toLocaleString()}</span>
                                </div>
                            </div>
                        </div>
                    </div>
