...
router.patch('/:id/details', auth, async (req, res) => {
    try {
        ...
        console.log(`Actualizando venta ${id}:`, { customerName, note });

        const sale = await prisma.sale.findUnique({ where: { id } });
        if (!sale) return res.status(404).json({ success: false, message: 'Venta no encontrada' });

        // Asegurarse de que customer sea un objeto
        let customer = {};
        try {
            customer = typeof sale.customer === 'string' ? JSON.parse(sale.customer) : (sale.customer || {});
        } catch (e) {
            customer = {};
        }
        ...
        console.log('Venta actualizada con éxito:', updatedSale.id);
