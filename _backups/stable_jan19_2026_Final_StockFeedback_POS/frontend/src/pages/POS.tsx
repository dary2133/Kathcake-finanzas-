                                ...
                                <button
                                    onClick={() => {
                                        if (availableStock > 0) {
                                            ...
                                            toast.error('Has alcanzado el límite del stock disponible');
                                        } else {
                                            toast.error('Producto agotado');
                                        }
                                    }}
                                    className={`w-full bg-white rounded-[2rem] p-4 flex flex-col gap-3 shadow-[0_10px_30px_-15px_rgba(0,0,0,0.05)] hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)] transition-all duration-500 active:scale-95 text-center overflow-hidden border border-slate-100 hover:border-primary-100 ${availableStock <= 0 ? 'opacity-60 grayscale-[0.5]' : ''}`}
                                >
                                    ...
                                    <div className="flex flex-col gap-1 items-center pb-2">
                                        ...
                                        <div className={`mt-2 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${availableStock > 0
                                            ...
                                            {Math.max(0, availableStock)} disponibles
                                        </div>
                                    </div>

                                    {/* Botón flotante opcional de agregar (micro-animación) */}
                                    <div className={`absolute bottom-4 right-4 opacity-0 group-hover:opacity-100 scale-50 group-hover:scale-100 transition-all duration-500 ${availableStock <= 0 ? 'hidden' : ''}`}>
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center gap-4 mt-3">
                                        <div className="flex items-center bg-slate-50 p-1 rounded-xl">
                                            ...
                                            <button
                                                onClick={() => {
                                                    if (item._id.toString().startsWith('free-') || item.quantity < item.stock) {
