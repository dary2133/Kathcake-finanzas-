...
router.patch('/:id', auth, checkPermission('canCreateSales'), async (req, res) => {
    try {
        const { id } = req.params;
        const { initialCash, countedCash, notes, status, openedAt, closedAt } = req.body;
        ...
        const newInitialCash = initialCash !== undefined ? parseFloat(initialCash) : session.initialCash;
        const newCountedCash = countedCash !== undefined ? parseFloat(countedCash) : session.countedCash;

        // Recalcular expectedCash si cambió initialCash
        let newExpectedCash = session.expectedCash;
        if (initialCash !== undefined) {
            const diff = newInitialCash - session.initialCash;
            newExpectedCash = session.expectedCash + diff;
        }

        // Recalcular discrepancy
        const newDiscrepancy = newCountedCash - newExpectedCash;

        const updatedSession = await prisma.cashRegisterSession.update({
            where: { id },
            data: {
                initialCash: newInitialCash,
                countedCash: newCountedCash,
                expectedCash: newExpectedCash,
                discrepancy: newDiscrepancy,
                notes: notes !== undefined ? notes : session.notes,
                status: status !== undefined ? status : session.status,
                openedAt: openedAt ? new Date(openedAt) : session.openedAt,
                closedAt: closedAt ? new Date(closedAt) : session.closedAt
            }
        });

        res.json({ success: true, message: 'Sesión actualizada correctamente', data: updatedSession });
    } catch (error) {
        console.error('Error updating register session:', error);
        res.status(500).json({ success: false, message: error.message });
    }
});
